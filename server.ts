...
function sanitizePersonalInfo(text: string): string {
  if (!text) return "";
  let sanitized = text;
  ...
  sanitized = sanitized.replace(/(?:tên em là|em tên là|mình tên là|tôi tên là|tên tôi là)\s+([A-ZÀ-Ỹa-zà-ỹ\s]+?)(?=[,.\n]|$)/gi, "em");

  // Mask school mentions: "trường THCS [Tên]", "em học trường..."
  sanitized = sanitized.replace(/(?:em học trường|học sinh trường|học trường|trường THCS|trường tiểu học|trường)\s+([A-ZÀ-Ỹa-zà-ỹ0-9\s]+?)(?=[,.\n]|$)/gi, "ở trường");

  // Mask address mentions: "nhà em ở...", "địa chỉ nhà ở..."
  sanitized = sanitized.replace(/(?:nhà em ở|địa chỉ ở|địa chỉ nhà|sống tại)\s+([A-ZÀ-Ỹa-zà-ỹ0-9\s,]+?)(?=[.\n]|$)/gi, "ở nhà");

  return sanitized;
}

// Check if a situation description is overly vague or lacks cause/context
function isSituationVague(text: string): boolean {
  ...
  if (trimmed.length < 35) {
    const contextKeywords = [
      "kiểm tra", "thi", "điểm", "bài tập", "ôn bài", "thầy", "cô", "giáo viên",
      "bạn", "cãi", "nhóm", "lớp", "học", "phát biểu", "bố", "mẹ", "gia đình",
      "tẩy chay", "hiểu lầm", "dự án", "thuyết trình", "lo vì", "buồn vì", "sợ vì"
    ];
...
function generateSafeFallback(emotionId: string, situation: string, clarificationAnswer?: string) {
  ...
  if (detectCrisis(combinedText)) {
    return {
      ...
      supportRecommendation: "Bạn hãy gọi ngay tới Tổng đài Quốc gia Bảo vệ Trẻ em 111 (miễn phí 24/7) hoặc nói chuyện với người lớn để nhận được hỗ trợ kịp thời nhé.",
      isCrisis: true,
    };
  }
  ...
  if (!trimmedAnswer && isSituationVague(trimmedSituation)) {
    ...
    return {
      ...
      suggestions: [], // Chưa đưa gợi ý vội khi chưa rõ tình huống
      supportRecommendation: "Bạn hãy trả lời câu hỏi nhỏ phía trên để SafeTalk đưa ra những gợi ý phù hợp nhất cho bạn nhé.",
      isCrisis: false,
    };
  }
...
app.post("/api/safetalk", async (req: Request, res: Response): Promise<void> => {
  try {
    const { emotion, situation, clarificationAnswer } = req.body;

    if (!emotion || !situation || typeof situation !== "string" || !situation.trim()) {
      res.status(400).json({ error: "Vui lòng chọn cảm xúc và chia sẻ ngắn gọn tình huống đang gặp." });
      return;
    }

    // 1. Sanitize all user inputs immediately for privacy
    const cleanSituation = sanitizePersonalInfo(situation.trim());
    const cleanClarification = clarificationAnswer && typeof clarificationAnswer === "string"
    ...
    if (detectCrisis(combinedSituation)) {
      res.json(generateSafeFallback(emotion, cleanSituation, cleanClarification));
      return;
    }
    ...
    if (!ai) {
      // Return safe high-quality fallback when GEMINI_API_KEY is not configured
      const fallback = generateSafeFallback(emotion, cleanSituation, cleanClarification);
      res.json(fallback);
      return;
    }
...
- Nếu tình huống đã đủ rõ (có chi tiết kiểm tra, mâu thuẫn bạn bè, bài tập...):
  ...
  * suggestions: đúng 3 gợi ý cụ thể, mỗi gợi ý đúng 1 câu ngắn.`;
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            needsClarification: {
              type: Type.BOOLEAN,
              description: "True nếu tình huống quá mơ hồ và cần hỏi 1 câu làm rõ duy nhất; False nếu đã đủ rõ hoặc đã trả lời làm rõ",
            },
            understanding: {
              type: Type.STRING,
            ...
            clarifyingQuestion: {
              type: Type.STRING,
              description: "Nếu needsClarification=true: đúng 1 câu hỏi ngắn; Nếu needsClarification=false: chuỗi rỗng",
            },
            suggestions: {
              type: Type.ARRAY,
              items: { type: Type.STRING },
              description: "Phần 3: Bạn có thể thử - mảng rỗng nếu needsClarification=true; tối đa 3 gợi ý nếu false",
            },
    ...
    // Helper to split and limit sentences
    ...
    const { emotion, situation, clarificationAnswer } = req.body || {};
    const fallback = generateSafeFallback(emotion || "lo_lang", situation || "", clarificationAnswer || "");
    res.json(fallback);
  }
});
